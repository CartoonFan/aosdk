...
static void vchan_event_push(vchan_t *vchan, event_type_t type,
                             event_param_t param) {
...
/// MIDI output
/// -----------
static FILE *midi_open(const char *fn) { return fopen_derivative(fn, ".mid"); }
...
static void midi_track_write(FILE *midi, const vchan_t *vchan,
                             char midi_channel) {
  const event_t *event;
  uint32 time_prev = 0;
...
ao_bool mididump_write(const char *fn) {
  ...
  qsort(vchans_sorted, i, sizeof(*vchans_sorted),
  ...
  if (beat_distance == 0) {
    beat_distance = 22050;
    printf("(piece too slow, falling back to %.2f BPM)\n",
...
void mididump_free(void) { vchans_free(); }
/// ----------
