...
static void wavedump_LIST_adtl_labl_write(wavedump_t *wave, uint32 point_id,
        const char *label) {
    RIFFCHUNK cLIST;
...
static void wavedump_header_fill(WAVEHEADER *h, uint32 data_size,
                                 uint32 file_size, uint32 sample_rate,
                                 uint16 bits_per_sample, uint16 channels) {
    ...
    h->Format.nAvgBytesPerSec =
        LE32(h->Format.nSamplesPerSec * h->Format.nBlockAlign);
...
void wavedump_finish(wavedump_t *wave, uint32 sample_rate,
                     uint16 bits_per_sample, uint16 channels) {
    assert(wave);
    if (wave->file) {
        WAVEHEADER h;
        ...
        wavedump_header_fill(&h, wave->data_size, ftell(wave->file), sample_rate,
