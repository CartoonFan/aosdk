...
void debug_memory(const char *view_id, DebugMemoryState *state, uint8 *mem_buf,
    ...
    enum ByteColors : uint8 { Normal, Control, MAX };
    ...
    ImGui::DragIntClamp("##rows", &state->rows, 0.2f, 1, ROWS_MAX,
                        "%.0f bytes per line");
    ...
    if (state->rows > 1) {
        ImGui::SameLine();
        ImGui::DragIntClamp("##offset", &state->offset, 0.2f, 0, state->rows - 1,
                            "Offset: %.0f");
    }
    ImGui::PopItemWidth();
    ...
    for (int i = 0; i < countof(vline_x); i++) {
        ImGui::GetWindowDrawList()->AddLine(ImVec2(vline_x[i], 0.0f),
                                            ImVec2(vline_x[i], vline_y_end),
...
ao_bool debug_frame(debug_hw_t *hw) {
    ...
    auto status_bar_pos =
        ImVec2(0.0f, io.DisplaySize.y - ImGui::GetWindowContentRegionMin().y);
    auto progress =
        (double)corlett_sample_count() / (double)corlett_sample_total();
    ...
    ImGui::Columns(2);
    ImGui::SetColumnOffset(1, ImGui::GetWindowContentRegionWidth() -
                           ImGui::CalcTextSize(fps).x);
    ImGui::NextColumn();
